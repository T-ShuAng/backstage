  ...
  <div class="entrepreneur list">
    ...
    <table class="list-table" cellspacing="0px">
      ...
      <tr class="list-row" v-for="(item,index) in list">
        <td><img :src="'/public/static/'+item.major_image"></td>
        <td><router-link :to="'/entre/audit/'+item.entr_id">{{item.user_name}}</router-link></td>
        <td class="t-long">{{item.company_name}}</td>
        <td>{{item.entr_position}}</td>
        <td>{{item.resident_area | getStr('areaArr')}}</td>
        <td>{{item.reg_time | getTime}}</td>
        <td>{{item.is_prove | getStr('proveArr')}}</td>
        <td><router-link :to="'/entreAudit/edit/'+item.entr_id">编辑</router-link></td>
      </tr>
    </table>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      // 请求函数
      getList(){
        axios.post('/apis/jcwyadminphp/demand/entrepreneurshipList',{
