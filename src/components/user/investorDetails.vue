<template>
  <div class="investorDetails detail-box">
    ...
    <div class="detail-center">
      ...
      <p><span class="span-title">资料注册时间</span><span>：{{detail.reg_time | getTime(true)}}</span></p>
      ...
      <div class="detail-center-box" v-for="item in detail.work">
        <p><span class="span-title">工作时间</span><span>：{{item.work_start_time | getTime(1)}}</span></p>
      ...
      <h3 class="detail-title">投资经历</h3>
      <div class="detail-center-box" v-for="item in detail.experience">
        <p><span class="span-title">投资时间</span><span>：{{item.experience_start_time | getTime(1)}}</span></p>
        <p><span class="span-title">投资内容</span><span>：{{item.experience_detail}}</span></p>
      </div>

      <p>
        <router-link :to="'/investor/audit/'+detail.id+'?type=user_id'" class="bottom-label">身份认证</router-link>
        <router-link :to="'/capital/list?id='+detail.id" class="bottom-label">发布的资金</router-link>
        <router-link :to="'/deliver/list?type_id='+detail.id" class="bottom-label">收到的BP</router-link>
      </p>
