  ...
  <div class="capital list">
    <table class="list-table" cellspacing="0px">
      ...
      <tr class="list-row" v-for="(item,index) in list">
        <td><img :src="'/public/static/'+item.company_logo" alt=""></td>
        <td>{{item.user_name}}</td>
        <td>{{item.user_sex | getStr('sexArr')}}</td>
        <td class="t-long"><router-link :to="'/capital/details/'+item.id">{{item.capital_title}}</router-link></td>
        <td>{{item.demand_time | getTime}}</td>
        <td>{{item.is_delete | getStr('isArr')}}</td>
     ...
     <!--   <td>{{item.stop | getStr('isArr')}}
        ...
        <td><router-link :to="'/capital/edit/'+item.id">编辑</router-link></td>
      </tr>
    </table>
    ...
    methods:{
     ...
     /* //是否中止筛选
      ...
      postStop(id,val,index){
        ...
        axios.post('/apis/jcwyadminphp/demand/capitalstop',{
          id,
          stop
        }).then((res)=>{
      ...
      // 请求函数
      getList(){
        let url = '/apis/jcwyadminphp/demand/capitallist';
        axios.post(url,{
          id:this.id,
